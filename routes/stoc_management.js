...
router.get("/product/:id", async (req, res) => {
    ...
    try {
        const stock = await stock_schema.findById(productId);
        const stockHistory = await StockHistory.find({ productId: productId }).sort({ date: -1 });
        const orderHistory = await Order.find({ "orderList.productId": productId }).sort({ date: -1 });
        ...
        res.render("stock_management/product", { id, name, email, stock, stockHistory, orderHistory });
    } catch (error) {
        console.error(error);
        res.redirect("/stock_management");
    }
